document.addEventListener('DOMContentLoaded', () => {
  ...
  for (let i = 0; i < boxs.length; i++) {
    boxs[i].addEventListener("click", (e) => {
      clearBorder();
      let parent = e.target.closest(".modal-inner");
      if (parent) {
        ...
        document.querySelector(".continue-btn").addEventListener("click", displayThanks);
      }
    });
  }
  ...
  select_btn.forEach(btn => btn.addEventListener("click", displayThanks));
  ...
  bookmark.addEventListener("click", () => {
    ...
    bookmark_text.innerText = bookmarked ? "Bookmarked" : "Bookmark";
  });
});
